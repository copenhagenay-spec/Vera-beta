...
Command Macros let you chain multiple voice commands into one phrase. Each step runs in order, waiting for SH|RA to finish speaking before moving to the next.
...
You define a trigger phrase and a list of steps. When you say the phrase, SH|RA runs each step in sequence — opening apps, reading the weather, checking reminders, whatever you set up.

> **Example:** Say "good morning" → SH|RA opens Spotify, reads the weather, then lists your reminders — each one finishing before the next starts.
...
5. Enter a **Step** — any voice command SH|RA understands (e.g. `open spotify`)
...
Any command SH|RA understands can be a macro step:
...
> **Tip:** Steps run in the order you add them. SH|RA waits for TTS to finish before starting the next step, so nothing overlaps.
...
**"SH|RA says the phrase but nothing happens"**
