"""Setup wizard UI for SHRA (PySide6)."""
...
def _create_shortcut():
    try:
        ...
        shortcut_path = os.path.join(desktop, "SHRA.lnk")
        start_path = os.path.join(start_menu, "SHRA.lnk")

        for stale in [
            os.path.join(start_menu, "SHRA.lnk"),
            os.path.join(start_menu, "SHRA (2).lnk"),
            os.path.join(desktop, "SHRA (2).lnk"),
        ]:
            try:
                os.remove(stale)
...
def run_wizard(
    ...
    dialog.setWindowTitle("SHRA Setup Wizard")
    ...
    header_title = QLabel("Welcome to SHRA!")
    ...
    # --- Model info ---
    info_lbl = _lbl(
        "SHRA uses Whisper for speech recognition. The model (~150MB) will download automatically "
    ...
    def _finish():
        ...
        dialog.accept()
        _start_background()
        QMessageBox.information(None, "Done", "Setup complete. SHRA is ready.")
